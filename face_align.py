...
class FaceAligner:
    def __init__(self, size=224, padding=0.25, device='cuda'):
        mean_face_shape_x = np.array(
        ...
        x = ((mean_face_shape_x + padding) / (2.0 * padding + 1)) * size
        y = ((mean_face_shape_y + padding) / (2.0 * padding + 1)) * size
    ...
    def align(self, image: np.ndarray) -> np.ndarray:
        point = self.fa.get_landmarks(image)[0]
